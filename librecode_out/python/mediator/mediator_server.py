...
Registers these routes on the main Flask app:
  ...
  GET  /mediator/scripts                    List available scripts in librecode/scripts/
...
def _get_scripts_dir() -> str:
    ...
    d = os.path.join(get_librecode_dir(), "scripts")
...
def seed_default_scripts():
    """Seed bundled scripts/ into librecode/scripts/ on first install only.
    ...
    if want_stream and stream_selector:
        ...
        def _generate():
            ...
            try:
                ...
                # recognizable by the LibreCode UI).  Falls back gracefully to
