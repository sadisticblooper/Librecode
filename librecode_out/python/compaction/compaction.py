"""
Context compaction for LibreCode Android.

How it works (mirrors librecode desktop logic):
...
Constants mirror librecode desktop values (scaled for mobile budgets):
...
# ── prompt loading ─────────────────────────────────────────────────────────────
# __file__ is now python/compaction/compaction.py — prompts live in the same dir
...
def prune_tool_outputs(messages: list) -> list:
    ...
    Logic (mirrors librecode desktop):
