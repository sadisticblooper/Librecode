...
function renderDiffSummary(diffs) {
    ...
    const rows = diffs.map(d => {
        ...
        const aCls    = added   ? 'diff-added'   : '';
        const rCls    = removed ? 'diff-removed' : '';
        return '<tr>' +
            '<td class="diff-file">' + escHtml(d.file || '') + '</td>' +
            '<td class="diff-cell ' + aCls + '">' + (added   ? '+' + added   : '') + '</td>' +
            '<td class="diff-cell ' + rCls + '">' + (removed ? '-' + removed : '') + '</td>' +
            '</tr>';
    }).join('');
    div.innerHTML =
        '<div class="diff-header">changed files</div>' +
